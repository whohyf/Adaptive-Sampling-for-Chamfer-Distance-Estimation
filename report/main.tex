...
boundary remains genuinely difficult: on the borderline subset, a
representative pooled threshold near $\sqrt{\log n}$ yields a
$15\%$ false-positive rate and a $50\%$ false-negative rate against
the Bernstein oracle, reflecting both pilot noise and the fact that
the deployed rule ignores $M(\gamma)$. End-to-end, on three fixed
synthetic demonstrators, the adaptive rule nearly matches uniform on
low-CV text-like data and reaches IS-level error on our ShapeNet-like
setting, while a planted-outlier dataset exposes a different
limitation: routing by estimated cost and achieving low realized error
can point in different directions. These results suggest that a
pilot-based rule is a useful practical heuristic, but do not support a
sharp or fully automatic resolution of the sampling-regime boundary.
\end{abstract}
...
\begin{itemize}
    ...
    a representative pooled threshold near $\sqrt{\log n}$ incurs a
    $15\%$ false-positive rate and a $50\%$ false-negative rate on the
    borderline subset relative to the Bernstein oracle. We interpret
    these rates as end-to-end routing disagreement, not as pure pilot
    estimation error, because the runtime rule intentionally ignores
    $M(\gamma)$.
    ...
    sampling. On our fixed low-CV text-like demonstrator, the rule
    correctly collapses to the uniform path and nearly matches its cost
    and error. On our fixed ShapeNet-like setting, it reaches IS-level
    mean estimation error, although pure IS is slightly lower in the
    canonical run. On a planted-outlier dataset, however, the
    experiments expose a different limitation: cost-based routing can
    still favor the uniform/brute-force side even when realized
    estimation quality is poor, so the pilot does not resolve the
    regime boundary as much as reveal it.
\end{itemize}
...
the pilot estimate $\widehat{\mathrm{CV}}_s$. As a result, disagreement
with the Bernstein oracle should be interpreted as end-to-end routing
disagreement of the deployed one-statistic rule, not as pure pilot-CV
estimation error: some mismatches are caused by pilot noise, while
others are caused by the deliberate omission of $M(\gamma)$ and the
brute-force cap from the runtime decision rule.
...
rising false-negative rate. As a representative pooled reference for
this mixed-$n$ benchmark, the closest grid value to $\sqrt{\log 2000}$
is $\tau=2.87$; this is not exactly the deployed default, which uses
the per-instance threshold $\tau=\sqrt{\log n}$. At this pooled
reference point, the
...
exact fallback is triggered on $7/20$ trials. In this case the reason
is not literally ``small $n$,'' but rather that the conservative
Chebyshev-budgeted uniform branch can still hit the exact cap on a
finite instance, making exact computation competitive in the
query-equivalent accounting even though the asymptotic motivation for
IS remains intact.
...
Overall, Table~\ref{tab:e2e} supports a nuanced conclusion on these
three fixed synthetic demonstrators. The adaptive rule behaves as
intended on the low-CV row, is competitive with IS on the
ShapeNet-like row despite conservative fallback behavior, and cleanly
exposes the limitations of pilot-based routing on the extreme
heavy-tail row.
...
Moreover, because our runtime rule intentionally uses only
$\widehat{\mathrm{CV}}_s$ while the Bernstein oracle also depends on
$M(\gamma)$ and the brute-force cap, the observed FP/FN rates conflate
pilot noise with model mismatch between the deployed heuristic and the
full cost model.
...
often recovers a reasonable branch choice on easy-to-moderate
synthetic instances. Just as importantly, the experiments make clear
where this strategy fails: on borderline instances and on extreme
heavy-tail data, the pilot does not eliminate uncertainty so much as
expose it.
